...
int main( int argc, char** argv )
{
    ...
    *******************************/
    ...
    for ( currIndex=startIndex+1; currIndex<endIndex; currIndex++ )
    {
       ...
       imshow("curFrame.rgb",currFrame.rgb);
        if (cv::waitKey(33) == 27)  //waitKey()至少创建一个HighGUI窗口才执行
          break ;
    }
    ...
    for (size_t i=0; i<keyframes.size(); i++)
    {
        ...
        // 把点云变换后加入全局地图中
        pcl::transformPointCloud( *newCloud, *tmp, pose.matrix() ); //与slambase.cpp 的joinpointcloud 有差异
        *output += *tmp;
        tmp->clear();
        newCloud->clear();
    }
...
CHECK_RESULT checkKeyframes( FRAME& f1, FRAME& f2, g2o::SparseOptimizer& opti, bool is_loops)
{
    ...
    // 如果是非回环检测情况则，向g2o中增加这个顶点与上一帧联系的边
    // 顶点部分
    // 顶点只需设定id即可
    if (is_loops == false) //判断是否为回环检测情况
    {
        g2o::VertexSE3 *v = new g2o::VertexSE3();
        v->setId( f2.frameID ); //设定节点id
...
void checkRandomLoops( vector<FRAME>& frames, FRAME& currFrame, g2o::SparseOptimizer& opti )
{
    ...
    srand( (unsigned int) time(NULL) ); //利用系统时间改变系统的种子值
    // 随机取一些帧进行检测
